    ...
    switch (message.type) {
      ...
      case "mouseButtons":
        ...
        if (Number.isFinite(message.leftInset)) {
          ...
          if (left !== viewportLeftInset) {
            viewportLeftInset = left;
            settlePan();
            render();
          }
        }
        ...
        const keyboardInset =
          fillHeight && message.keyboardOpen === true ? viewportBottomInset : 0;
        if (keyboardInset !== keyboardViewportInset) {
          ...
          if (fillHeight && (keyboardInset > 0 || wasOpen)) {
            ...
            const r = layout(),
              v = cursorViewport();
            place(
              (v.minX + v.maxX) / 2 - cx * r.width,
              (v.minY + v.maxY) / 2 - cy * r.height,
              r,
            );
            followRest = { fx, fy, zoom };
            cursorNeedsEntry = false;
