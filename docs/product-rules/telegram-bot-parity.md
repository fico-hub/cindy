...
| 2 | **msg.op 已接表情与定时主动正文，普通轮次其余动词待接** | 服务端全套动词在 `xindong/cindy-server#349`（未合）；桌面侧 `react` 继续用于 ack 表情；`hook-control/telegramDelivery.ts` + `manager.ts` 为定时结果与明确授权测试接入 `send`（HTML / plain），经 `scheduler/telegramDelivery.ts` 暴露 status / send / receipt。只选服务端曾下发、属于当前绑定 principal 与 bot 的本人 DM；双方显式协商 `telegram-dm-send-v1` 并带进程 epoch 后才发，`msg-op-v1` 单独只证明表情能力；持久 started 先于网络写，真实 messageId 和实际目标匹配才算 sent，未知结果不重发，迟到回执可更新原记录。普通轮次的 `send` / `edit` / `delete` / `typing` / `media` 仍未接线。配套回执含 Telegram 实际正文/entities/chatId/tier；Host 不代替调用者比较展示稿，故 `formatVerified=false`。源码测试通过不证明服务端已上线或实发验真 | #1855 第三刀。**这是把官方 bot 的出站改由桌面驱动的关键一步**——接完之后两侧的发射与收口才可能走同一份代码，而不是各写一套 |
