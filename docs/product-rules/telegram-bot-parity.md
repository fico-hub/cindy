...
| 终态错误的**可读映射**（过载耗尽、model 与来源不匹配） | `im/shared/turnRetryNotice.ts`（`overloadFailureNotice` + `providerModelMismatchNotice`） | 把 `Selected model is at capacity...`、`provider '…' cannot serve model '…'`（#2141，渠道默认 provider 切 BYOM 后旧会话路由被拒）这类内部串换成可执行说明（重发重试 / 发送 `/model` 重选 / 发送 `/new` 另起上下文）的**判定与文案**两侧同一份：个人侧经 `terminalErrorText` 在 `turnRunner` 的两条收口路径（用户 turn + 调度转播）消费；官方侧桌面半在 `hook-control/turnObserver.ts` 消费同两份映射后经 `errorMessage` 字段交服务端渲染前缀。**不命中映射的错误沿用上游原文**——映射只增不改语义，上游原文两侧都留在本地日志 |
